...
func (t *ImmutableTree) Get(key []byte) []byte {
	...
	if err != nil {
		logger.Debug("failed to get FastNode with key: %X, falling back to regular IAVL logic\n", key)
	...
	if fastNode == nil {
		...
		if t.version == t.ndb.latestVersion {
			logger.Debug("latest version with no fast node for key: %X. The node must not exist, return nil. Tree version: %d\n", key, t.version)
			return nil
		}

		logger.Debug("old version with no fast node for key: %X, falling back to regular IAVL logic. Tree version: %d\n", key, t.version)
		_, result := t.root.get(t, key)
		return result
	}
	...
	if fastNode.versionLastUpdatedAt > t.version {
		logger.Debug("last updated version %d is too new for FastNode where tree is of version %d with key %X, falling back to regular IAVL logic\n", fastNode.versionLastUpdatedAt, t.version, key)
		_, result := t.root.get(t, key)
		return result
	}
